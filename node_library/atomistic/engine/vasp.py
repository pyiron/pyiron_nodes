...
def write_POTCAR(workdir: str, vasp_input, filename: str = "POTCAR") -> str:
    ...
    if vasp_input.potcar_paths is None:
        potcar_paths = get_default_POTCAR_paths(vasp_input.structure, vasp_input.pseudopot_lib_path)
    else:
...
def write_KPOINTS(workdir: str, kpoints: Optional[Kpoints] = None, filename: str = "KPOINTS") -> str:
...
def create_WorkingDirectory(workdir: str, quiet: bool = False) -> str:
    ...
    if not os.path.exists(workdir):
        ...
        warnings.warn(f"Directory '{workdir}' already exists. Existing files may be overwritten.")
    return workdir
...
class Storage:
    ...
    def _convert_to_dict(instance) -> Dict:
        ...
        result_dict = {key: value for key, value in attributes.items() if "_" not in key[0]}
        return result_dict
...
def run_job(command: str, workdir: str = os.getcwd(), environment: Optional[Dict[str, str]] = None, arguments: Optional[List[str]] = None) -> ShellOutput:
...
def check_convergence(workdir: str, filename_vasprun: str = "vasprun.xml", filename_vasplog: str = "vasp.log", backup_vasplog: str = "error.out") -> bool:
    converged = False
    ...
    try:
        ...
        try:
            converged = is_line_in_file(filepath=os.path.join(workdir, filename_vasplog), line=line_converged, exact_match=False)
        except:
            try:
                converged = is_line_in_file(filepath=os.path.join(workdir, backup_vasplog), line=line_converged, exact_match=False)
            except:
                pass
...
def vasp_job(self, workdir: str, vasp_input, command: str = "module load vasp; module load intel/19.1.0 impi/2019.6; unset I_MPI_HYDRA_BOOTSTRAP; unset I_MPI_PMI_LIBRARY; mpiexec -n 1 vasp_std"):
    ...
    self.working_dir >> self.vaspwriter >> self.job >> self.vasp_output >> self.convergence_status
...
def get_default_POTCAR_paths(structure: Structure, pseudopot_lib_path: str, potcar_df: pd.DataFrame = pd.read_csv(POTCAR_specification_data)) -> List[str]:
    ele_list, _ = stack_element_string(structure)
    potcar_paths = []
    for element in ele_list:
        ele_default_potcar_path = potcar_df[(potcar_df["symbol"] == element) & (potcar_df["default"] == True)].potential_name.values[0]
        potcar_paths.append(os.path.join(pseudopot_lib_path, ele_default_potcar_path, "POTCAR"))
