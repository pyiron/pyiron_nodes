...
from pyiron_nodes.dpg2026.atomistic.assyst.stoichiometry import SpaceGroupSampling
...
wf.RelaxFull = Relax(
    structure="NotData",
    engine=wf.GRACE,
    opt_parameters=wf.GenericOptimizerSettings_1,
    opt_mode="full",
)
wf.RelaxFull.inputs.add(
    "store", port_type=bool, default=False, value=False, has_explicit_default=True
)

wf.SpaceGroupSampling = SpaceGroupSampling(
    elements=wf.Multiply, max_atoms=2, max_structures=5
)
wf.SpaceGroupSampling.inputs.add(
    "store", port_type=bool, default=False, value=True, has_explicit_default=True
)

wf.GetStaticEnergies = MapCalculatorOnStructures(
    structures=wf.SpaceGroupSampling, calculator=wf.Static, store_structures=True
)
wf.GetStaticEnergies.inputs.add(
    "store", port_type=bool, default=False, value=True, has_explicit_default=True
)

wf.GetRelaxedConstVolEnergies = MapCalculatorOnStructures(
    structures=wf.SpaceGroupSampling, calculator=wf.Relax, store_structures=True
)
wf.GetRelaxedConstVolEnergies.inputs.add(
    "store", port_type=bool, default=False, value=True, has_explicit_default=True
)

wf.GetRelaxedEnergies = MapCalculatorOnStructures(
    structures=wf.SpaceGroupSampling, calculator=wf.RelaxFull, store_structures=True
)
...
wf.GetColumnFromDataFrame = GetColumnFromDataFrame(
    df=wf.GetStaticEnergies, column_name="energy", as_array=True
)

wf.GetColumnFromDataFrame_1 = GetColumnFromDataFrame(
    df=wf.GetRelaxedEnergies, column_name="energy"
)

wf.HistogramStatic = Histogram(x=wf.GetColumnFromDataFrame, options=wf.InputPlotOptions)

wf.HistogramFull = Histogram(
    x=wf.GetColumnFromDataFrame_1, options=wf.InputPlotOptions_1
)

wf.Scatter = Scatter(x=wf.GetColumnFromDataFrame, y=wf.GetColumnFromDataFrame_1)

wf.LinearFittingCurve = LinearFittingCurve(
    x=wf.GetColumnFromDataFrame, y=wf.GetColumnFromDataFrame_1
)
