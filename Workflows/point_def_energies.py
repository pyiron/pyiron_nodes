...
def CreateStructures(name, element, cubic=False, repeat_scalar=1):
    ...
    inner_wf.CreateVacancy_1 = CreateVacancy(
        structure_container=inner_wf.CreateVacancy, parent_defect_index=-1
    )
    inner_wf.CreateSubstitutional = CreateSubstitutional(
        element=element, structure_container=inner_wf.CreateVacancy_1
    )
    inner_wf.GetStructureTable = GetStructureTable(
        structure_container=inner_wf.CreateSubstitutional
    )
...
wf.CreateStructures = CreateStructures(
    name="Al", cubic=True, repeat_scalar=3, element="Ni"
)
...
wf.GetColumnFromDataFrame = GetColumnFromDataFrame(
    df=wf.CreateStructures, column_name="structure"
)
