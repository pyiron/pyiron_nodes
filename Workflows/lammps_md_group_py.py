...
def Lammps(calc_dataclass, structure, potential, working_directory='.'):
    from pyiron_nodes.atomistic.engine.lammps import CreateLammpsMDInput, CreateLammpsStructure, ParseLammpsOutput, RunLammpsCalculation
    from core import Workflow
    
    inner_wf = Workflow("Lammps")
    inner_wf.CreateLammpsStructure = CreateLammpsStructure(structure=structure, potential=potential, working_directory=working_directory)
    inner_wf.CreateLammpsMDInput = CreateLammpsMDInput(io_bundle=inner_wf.CreateLammpsStructure, calc_dataclass=calc_dataclass)
    inner_wf.RunLammpsCalculation = RunLammpsCalculation(io_bundle=inner_wf.CreateLammpsMDInput)
    inner_wf.ParseLammpsOutput = ParseLammpsOutput(io_bundle=inner_wf.RunLammpsCalculation.outputs.io_bundle)
...
wf.Lammps = Lammps(calc_dataclass=wf.InputCalcMD, structure=wf.FixSpecies, potential=wf.GetPotential, working_directory='./test')
