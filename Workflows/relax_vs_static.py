...
wf.CreateStrainedStructures = IterToDataFrame(node=wf.Bulk, input_label='a', values=wf.Linspace)
wf.CreateStrainedStructures.inputs.add("debug", port_type=bool, default=False, value=False, has_explicit_default=True)
wf.CreateStrainedStructures.inputs.add("executor", port_type=object, default=None, value=None, has_explicit_default=True)
wf.CreateStrainedStructures.inputs.add("store", port_type=bool, default=False, value=False, has_explicit_default=True)

wf.GetRelaxedEnergies = MapCalculatorOnStructures(structures=wf.CreateStrainedStructures, calculator=wf.Relax)
wf.GetRelaxedEnergies.inputs.add("store", port_type=bool, default=False, value=True, has_explicit_default=True)

wf.GetStaticEnergies = MapCalculatorOnStructures(structures=wf.CreateStrainedStructures, calculator=wf.Static)
wf.GetStaticEnergies.inputs.add("store", port_type=bool, default=False, value=True, has_explicit_default=True)

wf.PlotRelaxed = PlotDataFrame(df=wf.GetRelaxedEnergies, x='a', options=wf.InputPlotOptions)

wf.PlotDataFrame = PlotDataFrame(df=wf.GetRelaxedEnergies, x='a')

wf.PlotStatic = PlotDataFrame(df=wf.GetStaticEnergies, x='a', options=wf.InputPlotOptions_1)
