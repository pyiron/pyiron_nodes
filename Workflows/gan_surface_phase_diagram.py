...
def BuildSurfaceDefects(
    ...
    for seed in range(n_seeds):
        if len(ga_adatom_positions) > 0:
            ...
            rows.append(
                op_interstitial(base_row, element="Ga", positions=pos, n=1, seed=seed)
            )
        if len(n_adatom_positions) > 0:
            pos = [n_adatom_positions[seed % len(n_adatom_positions)].tolist()]
            rows.append(
                op_interstitial(base_row, element="N", positions=pos, n=1, seed=seed)
            )
...
def ChemicalPotentialSweep(
    mu_ref: float, delta_mu: float = -2.5, num_points: int = 200
):
...
def PlotDefectPhase(
    ...
    ef_matrix = np.array(
        [np.atleast_1d(np.asarray(formation_energies[k], dtype=float)) for k in keys]
    )
...
def PlotStableDecoratedStructures(
    ...
    stable = [
        (name, to_ase(name_to_struct[name]))
        for name in stable_names
        if name in name_to_struct
    ]
    if not stable:
        fig, ax = plt.subplots()
        ax.text(
    ...
    fig, axes = plt.subplots(
        rows_n, cols, figsize=(figure_size * cols, figure_size * rows_n), squeeze=False
    )
    ...
    for idx, (name, atoms) in enumerate(stable):
        ...
        def _atom_color(i, sym):
            if sym == "Ga":
                if is_ga_ad:
                    ga_zs = [
                        positions[j, 2] for j, s in enumerate(symbols) if s == "Ga"
                    ]
        ...
        if render_style == "ball+stick":
            _draw_bonds_2d(
                atoms, ax, rotation
            )  # drawn after atoms so lines (zorder=2) sit on top
...
def _build_view3d(
    ...
    for color, idx in groups.items():
        view.add_representation(
            ...
            color=color,
            radiusType="vdw",
            radiusScale=ball_scale * particle_size,
        )

    if render_style == "ball+stick":
        # NGL auto-bonds by distance; one grey stick representation covers all bonds
        view.add_representation(
            "licorice", selection="all", color="#808080", radius=0.12 * particle_size
        )

    if (
        show_cell
        and atoms.cell is not None
        and np.any(np.abs(np.array(atoms.cell)) > 1e-2)
    ):
        view.add_unitcell()
...
def View3DStructure(
    ...
    if stable_only and formation_energies is not None:
        ...
        stable_names = [keys[i] for i in sorted(set(hull_idx.tolist()))]
        name_to_struct = {
            row["name"]: row["structure"] for _, row in relaxed_df.iterrows()
        }
        stable_structs = [
            name_to_struct[n] for n in stable_names if n in name_to_struct
        ]
    ...
    if coloring == "element":
        ...
        colors = [
            (
                _element_color_hex("Ga")
                if s == "Ga"
                else _CNA_COLOR.get(int(lbl), "#c0c0c0")
            )
...
def compute_surface_formation_energies(relaxed_df, mu_ga_sweep, mu_gan_fu):
    ...
    inner.with_deltas = AddDefectConcentrationColumns(
        df=inner.with_counts, pristine_row=0
    )
...
wf.view_surface = Plot3D(
    structure=wf.surface, render_style="ball+stick", particle_size=1.0
)
...
wf.view_3d = View3DStructure(
    relaxed_df=wf.relaxed_surface_df,
    formation_energies=wf.formation_energies,
    index=0,
    coloring="element",
    stable_only=False,
