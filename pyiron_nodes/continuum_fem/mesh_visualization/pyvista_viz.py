...
@as_function_node("plotter")
def PlotDeformedMesh1DObject(
    function_space, solution_vector, warp_factor: Optional[float | int]
):
    import pyvista
    from dolfinx.plot import vtk_mesh
...
def PlotVonMises3DObject(
    ...
    s = three_d_sigma(solution_vector, C) - 1.0 / 3 * ufl.tr(
        three_d_sigma(solution_vector, C)
    ) * ufl.Identity(len(solution_vector))
