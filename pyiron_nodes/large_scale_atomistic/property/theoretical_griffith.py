...
def rotate_elasticity_tensor(
    ...
    Q = np.array(
        [
            [np.matmul(m1, e1), np.matmul(m1, e2), np.matmul(m1, e3)],
            [np.matmul(m2, e1), np.matmul(m2, e2), np.matmul(m2, e3)],
            [np.matmul(m3, e1), np.matmul(m3, e2), np.matmul(m3, e3)],
        ]
    )
    K1 = np.array(
        [
    ...
    C = np.dot(
        np.dot(KK, C_base), np.transpose(KK)
    )  # Rotation of the material stiffness tensor
...
def theor_K_griffith_plane_strain(
    ...
    """
    Returns the theoretical Griffith fracture toughness for a desired material under plane strain in MPa*sqrt(m).


        Parameters:
            C: the elasticity tensor rotated to desired orientation
            gamma_s: Surface energy in J/m^2
    """
    ...
    Estar = ((b11 * b22 / 2) * (np.sqrt(b22 / b11) + (2 * b12 + b66) / (2 * b11))) ** (
        -0.5
    )
