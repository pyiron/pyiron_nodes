...
@pytest.fixture
def binary_classification_data() -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]:
    """
    ...
    y_train = pd.Series(
        (X_train.iloc[:, 0] + X_train.iloc[:, 1] > 0).astype(int),
        name="target"
    )
    ...
    y_test = pd.Series(
        (X_test.iloc[:, 0] + X_test.iloc[:, 1] > 0).astype(int),
        name="target"
    )
...
@pytest.fixture
def multiclass_classification_data() -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]:
    """
...
def trained_rf_classifier(binary_classification_data):
    ...
    return RandomForestClassifierNode(X_train, y_train, n_estimators=10, random_state=42)
...
class TestLogisticRegressionNode:
    ...
    def test_custom_penalty(self, binary_classification_data):
        ...
        X_train, y_train, _, _ = binary_classification_data
        result = LogisticRegressionNode(X_train, y_train, penalty="l1", solver="liblinear")
...
class TestPredictClassification:
    ...
    def test_prediction_with_probabilities(self, binary_classification_data, trained_rf_classifier):
...
class TestReproducibility:
    ...
    def test_random_forest_reproducibility(self, regression_data):
        ...
        result1 = RandomForestRegressorNode(X_train, y_train, n_estimators=10, random_state=42)
        result2 = RandomForestRegressorNode(X_train, y_train, n_estimators=10, random_state=42)
