...
def validate_data(X: Union[pd.DataFrame, np.ndarray], 
...
def ValidateRegressionModels(
    ...
    if verbose:
        print(f"Validating {len(models)} regression models on test set ({len(X_test)} samples)...")
    ...
    for model_name, model in models.items():
        try:
            ...
            if verbose:
                print(f"✓ {model_name:30s} | R²={r2:7.4f} | RMSE={rmse:7.4f} | MAE={mae:7.4f}")
    ...
    if metric == "r2":
        results_df = all_metrics_df.sort_values("r2", ascending=False).reset_index(drop=True)
    else:
        results_df = all_metrics_df.sort_values(metric, ascending=True).reset_index(drop=True)
...
def ValidateClassificationModels(
    ...
    if verbose:
        print(f"Validating {len(models)} classification models on test set ({len(X_test)} samples)...")
        print(f"Primary metric: {metric}")
        print()
    ...
    for model_name, model in models.items():
        try:
            ...
            if verbose:
                print(f"✓ {model_name:30s} | Acc={accuracy:6.4f} | F1={f1:6.4f} | Prec={precision:6.4f} | Rec={recall:6.4f}")
    ...
    results_df = all_metrics_df.sort_values(metric, ascending=False).reset_index(drop=True)
    results_df["rank"] = range(1, len(results_df) + 1)
    ...
    return {
        "results_df": results_df[["rank", "model_name", "accuracy", "f1", "precision", "recall"]],
        "best_model_name": best_model_name,
        "best_score": best_score,
        "metric": metric,
        "all_metrics_df": all_metrics_df,
...
def GenerateDetailedValidationReport(
    ...
    if output_file:
        ...
        if verbose:
            print(f"Reports saved: {output_file}_detailed.csv, {output_file}_summary.csv")
...
def CompareModelPair(
    ...
    if task_type == "regression":
        ...
        if verbose:
            print(f"{model_1_name:30s}: R²={metrics_1['r2']:.6f}, RMSE={metrics_1['rmse']:.6f}, MAE={metrics_1['mae']:.6f}")
            print(f"{model_2_name:30s}: R²={metrics_2['r2']:.6f}, RMSE={metrics_2['rmse']:.6f}, MAE={metrics_2['mae']:.6f}")
            print()
        ...
        metrics_1 = {
            ...
            "precision": precision_score(y_test, y_pred_1, average="weighted", zero_division=0),
            "recall": recall_score(y_test, y_pred_1, average="weighted", zero_division=0),
        }
        metrics_2 = {
            "accuracy": accuracy_score(y_test, y_pred_2),
            "f1": f1_score(y_test, y_pred_2, average="weighted", zero_division=0),
            "precision": precision_score(y_test, y_pred_2, average="weighted", zero_division=0),
            "recall": recall_score(y_test, y_pred_2, average="weighted", zero_division=0),
        }
        ...
        winner = model_1_name if metrics_1["accuracy"] > metrics_2["accuracy"] else model_2_name
        comparison["winner"] = winner
        comparison["metric_used"] = "Accuracy"
        
        if verbose:
            print(f"{model_1_name:30s}: Acc={metrics_1['accuracy']:.6f}, F1={metrics_1['f1']:.6f}, Prec={metrics_1['precision']:.6f}, Rec={metrics_1['recall']:.6f}")
            print(f"{model_2_name:30s}: Acc={metrics_2['accuracy']:.6f}, F1={metrics_2['f1']:.6f}, Prec={metrics_2['precision']:.6f}, Rec={metrics_2['recall']:.6f}")
            print()
            print(f"Winner: {winner}")
