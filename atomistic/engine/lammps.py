...
def CreateLammpsStructure(
    ...
    else: # Check if this works for the case where potential is a string name of the potential, not a dataframe. Should be the case for regular EAM, etc
...
def RunLammpsCalculation(
    ...
    if isinstance(io_bundle.potential, pd.DataFrame): 
        with open(
            os.path.join(io_bundle.working_directory, io_bundle.lammps_potential_filename),
            "w",
        ) as f:
...
def ParseLammpsOutput(
    ...
    if out.positions is not None:
        ...
        all_symbols = io_bundle.structure.get_chemical_symbols()
